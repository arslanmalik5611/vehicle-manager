...
function purchase_load() {
    $.ajax({
        url: api_url + 'purchase/{{ request()->id }}',
        dataType: "JSON",
        success: function (response) {
            $('#date').val(reformatDatePickerDate(response.data.date));
            $('#supplier_id').val(response.data.supplier_id).change();
            tinyMCE.get('notes').setContent(response.data.notes);
            $(response.data.details).each(function (index, detail) {
                if (detail.expiry_date) {
                    detail.expiry_date = reformatDatePickerDate(detail.expiry_date);
                }
                create_item_row(detail);
            })
        }
    });
}
...
function create_item_row(item_data) {
    purchase_detail_id = '';
    if (item_data.id) {
        purchase_detail_id = item_data.id;
    }
    var newRow = `<tr class="item-row"  purchase-detail-id="${purchase_detail_id}">
...
function getFormData() {
    ...
    $('.item-row').each(function () {
        var item_row = {
            'purchase_detail_id': $(this).attr('purchase-detail-id'),
