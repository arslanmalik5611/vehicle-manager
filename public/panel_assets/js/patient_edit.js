$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#date_arrived').val(reformatDatePickerDate(response.data.date_arrived));
            $('#date_left').val(reformatDatePickerDate(response.data.date_left));
            $('#nhs_number').val(response.data.nhs_number);
            $('#ethnicity').val(response.data.ethnicity);
            $('#vaccination_status').val(response.data.vaccination_status).change();
            $('#phe_report_date').val(reformatDatePickerDate(response.data.phe_report_date));
            $('#received_at').val(reformatDatePickerDateTime(response.data.received_at));
            $('#collection_datetime').val(reformatDatePickerDateTime(response.data.collection_datetime));
            $('#collection_center_id').val(response.data.collection_center_id).change();
            $('#reference_id').val(response.data.reference_id).change();
            $('#paid-amount').val(response.data.paid_amount);
            $('#net-amount').text(response.data.net_amount);
            $('#due-amount').text(response.data.net_amount - response.data.paid_amount);
            $(response.data.patient_tests).each(function (index, elem) {
                var test_data = {
                    id: elem.test_id,
                    name: elem.test.name,
                    sample_type: elem.test.sample_type.name,
                    turn_around_time: elem.test.turn_around_time,
                    amount: elem.net_amount
                };
...
function create_test_row(test) {
    var newRow = `<tr>
                                    ...
                                    <td>
                                        <input type="hidden" class="selected_test_id" data-amount="${test.amount}" value="${test.id}">
...
function getFormData() {
    ...
    $('.selected_test_id').each(function () {
        data.tests.push({test_id: $(this).val(), 'amount': $(this).attr('data-amount')});
    });
