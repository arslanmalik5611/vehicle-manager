        ...
        <div class="card-body">
            <form method="post" id="form-data">
                ...
                <figure>
                    ...
                    <figcaption class="blockquote-footer">
                        Please fill in details below <cite title="Source Title">if want to leave</cite>
                    </figcaption>
                </figure>

                <div class="row mb-3">
                    <div class="col-sm-6 col-12">
                        <label for="leave_date" class="form-label"><span class="required"></span>Leave Date </label>
                        <input type="text" class="form-control datepicker" autocomplete="off" id="leave_date" name="leave_date"
                               placeholder="dd/mm/YY">
                        <br>
                        <label for="leave_notes" class="form-label">Notes </label>
                        <textarea name="leave_notes" id="leave_notes" class="form-control" placeholder="Reason of Leave"></textarea>
                    </div>
                </div>
            ...
            $(document).ready(function () {
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $('#religion').val(data.user.religion);
                        $('#address').val(data.user.address);
                        $('#joining_date').val(data.joining_date_formatted);
                        $('#qualification').val(data.qualification);
                        $('#session_id').val(data.session_id);
                        $('#designation').val(data.designation);
                        $('#salary').val(data.salary);
                        $('#image').attr('src', data.user.user_picture_url);
                        $('#role_id').val(data.user.role_id).change();
                        $('#leave_date').val(data.leave_date_formatted).change();
                        $('#leave_notes').val(data.leave_notes);
                        $('#bank_information').html(data.bank_information);

                        // $('.section_id').each(function () {
                        //     if ($(this).val() == response.data.session.id){
